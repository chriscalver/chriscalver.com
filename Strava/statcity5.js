...
async function getAccessTokens() {
  const request = await fetch(
    "https://www.strava.com/oauth/token?client_id=REPLACE_WITH_STRAVA_CLIENT_ID&client_secret=REPLACE_WITH_STRAVA_CLIENT_SECRET&refresh_token=REPLACE_WITH_STRAVA_REFRESH_TOKEN&grant_type=refresh_token",
    {
      method: "POST",
      headers: {
  ...
  async function getActivities() {
    ...
    workoutmins = workoutmins / 60;

    document.getElementById("WorkOutMins").innerHTML = workoutmins.toFixed(2);
    ...
    // 0.4799999999
    const movingtimedecimal60tosecondsrounded =
      movingtimedecimal60toseconds.toFixed(2); // rounds to 0.48
    const secondstrimmed = movingtimedecimal60tosecondsrounded
      .toString()
      .slice(2); // drops 0. leaves 48 *******
    ...
    let pace =
      paceraw2.toString() + ":" + pacerawdecimal2trimmed.toString().slice(2);
    ...
    if (data2[0].has_heartrate == false) {
      ...
      document.getElementById("HeartRate").innerHTML =
        data2[0].average_heartrate + " bpm";
    }

    // document.getElementById('Pace').innerHTML = pace + " mins/km";
    document.getElementById("ActivityTime").innerHTML =
      movingtimetrimmed3.toString() + ":" + minutes + ":" + secondstrimmed;
...
async function getCurrentConditions() {
  const endpoint = new URL(
    "https://dataservice.accuweather.com/currentconditions/v1/55489?apikey=REPLACE_WITH_ACCUWEATHER_API_KEY"
  );
  ...
  document.getElementById("Time").innerHTML =
    myDate.toLocaleTimeString("default");
  document.getElementById(
    "WeatherIcon"
  ).src = `https://www.accuweather.com/assets/images/weather-icons/v2a/${icon}.svg`; 
}
...
async function getHourly() {
  const hourlyEndpoint = new URL(
    "https://dataservice.accuweather.com/forecasts/v1/hourly/12hour/55489?apikey=REPLACE_WITH_ACCUWEATHER_API_KEY&metric=true"
  );
  ...
  while (i < 12) {
    ...
    document.getElementById(`Hour${i}`).innerHTML =
      hour1short + " " + " - " + Temp + "c" + " " + condition + " " + PoP + "%";
    i++;
  }
}
